...
class LlataEncoderWithAction(nn.Module):
    def __init__(self, observation_shape, action_size, feature_size, hidden_size):
        super().__init__()
        ...
        self.fc1 = nn.Linear(self.observation_size + self.action_size, self.hidden_size)
        self.fc2 = nn.Linear(self.hidden_size, self.feature_size)
    ...
    def forward(self, x, action):
        ...
        assert len(action.shape) == 2, 'action must be 2-dim tensor. (batch_size, action_size)'
...
class LlataEncoderFactory(EncoderFactory):
    ...
    def create_with_action(
        ...
        return LlataEncoderWithAction(observation_shape, action_size, self.feature_size, self.hidden_size)
...
def get_args():
    ...
    parser.add_argument("--agent_name", type=str, default="bc", help="The type of offlineRL agent.")
